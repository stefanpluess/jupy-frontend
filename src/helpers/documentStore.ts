...
import {Edge, Node, useNodes, useNodesState, useReactFlow, XYPosition} from "reactflow"
import {useState, useCallback} from 'react';
import {createWithEqualityFn} from 'zustand/traditional';
import useNodesStore from "./nodesStore";
import { Notebook } from "../config/types";
import {NotebookCell} from "../config/types";
...
export const useDocumentStore = createWithEqualityFn<documentStore>((set, get) => ({
    ...
    setDocument: (nodes: Node[], edges: Edge[]) => {
        set({nodeShadow: cloneDeep(nodes), edgesShadow: cloneDeep(edges)})
    },
    ...
    deleteNode: (node_id : string) => {
        set({nodeShadow : get().nodeShadow!.map(node => {
            return node.id !== node_id && node.id !== node_id + '_output' ? node : null;
